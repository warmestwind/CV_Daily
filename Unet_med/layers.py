...
def crop_and_concat(x1,x2):
    with tf.name_scope("crop_and_concat"):
        ...
        # offsets for the top left corner of the crop, 去中心部分
        offsets = [0, (x1_shape[1] - x2_shape[1]) // 2, (x1_shape[2] - x2_shape[2]) // 2, 0]
        size = [-1, x2_shape[1], x2_shape[2], -1]
        x1_crop = tf.slice(x1, offsets, size) # input begin size, 这里默认x1的size大于x2
        return tf.concat([x1_crop, x2], 3) # 沿channel连接
