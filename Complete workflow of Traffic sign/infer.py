...
#以上是infer需要复制的
    ...
    #result= sess.run(accuracy,feed_dict={ x:images_a , dropout1: 1.0})
    #print("result= ",result)
    yout=y_output.eval(feed_dict={x:images_a, dropout1:1.0})
    yout2=tf.argmax(yout,1)
    np.set_printoptions(threshold=np.inf) 
    f=open("out.txt","w")
    print(yout2.eval(),file=f)
    f.close()
    plt.show()
    '''
    #num of epochs
...
if __name__ == "__main__":
    print ('This is test_lenet module')
